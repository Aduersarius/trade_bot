    ...
    for i in range(0, 20, 2):
        ...
        for ticker in tickers:
            ...
            for episode in range(size):
                # try:
                    stats = run(Environment(data=copy.deepcopy(data[episode]), rsi=i))

                    print("Profit rsi:", "{:.2%}".format(stats[1] - 1), "  Actions:", stats[2])
                    print("Buy&hold:", "{:.2%}".format(stats[3]), "  Loss:", "{:.2%}".format(stats[3]),
                          "  diff:",
                          "{:.2%}".format(stats[5] - 1), " Profits balance:", "{:.2%}".format(stats[4]))
                    profits.append(stats[1] - 1)
                    diff.append(stats[5] - 1)
